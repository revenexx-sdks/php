...
class Client
{
    ...
    protected array $headers = [
        'content-type' => '',
        'user-agent' => 'RevenexxPHPSDK/0.1.3 ()',
     ...
     * @return Client
     */
    public function setMarket(string $value): Client
    {
        $this->addHeader('X-Revenexx-Market', $value);
    ...
    )
    {
        ...
        $warnings = $responseHeaders['x-revenexx-warning'] ?? '';
        if ($warnings) {
        ...
        if (curl_errno($ch)) {
            throw new RevenexxException(curl_error($ch), $responseStatus, $responseBody['type'] ?? '', $responseBody);
        }
